...
> **Tip:** Run `hostname` in your terminal to see your current hostname. Replace `#desktop` with `.#<your-hostname>` below.

```bash
# Check for build errors without creating the 'result' symlink
# Example for desktop:
nixos-rebuild build --flake .#desktop --no-link
...
1. **Copy Host:** `cp -r hosts/desktop hosts/new-machine`
